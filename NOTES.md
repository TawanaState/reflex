...
## [2026-09-23] Scalar early-exit extension: matched, negative-leaning result

Extended `NativeDraftObserver` (`src/engine/early_exit.py`) to exit on a fully specified typed-scalar call (every declared field including optional ones present, schema-valid, identical for N consecutive drafts), gated behind `REFLEX_SCALAR_EARLY_EXIT` (default `false`) and `REFLEX_SCALAR_STABLE_STEPS` (default `2`). Wired into `src/engine/runner.py` alongside the existing atomic gate; both re-validate arguments against the schema at accept time before returning `NATIVE_SCALAR_EARLY_EXIT`, so the observer's own bookkeeping is never trusted blindly. 10 new focused tests in `tests/test_native_early_exit.py` (20/20 total in that file including the pre-existing atomic tests) pass.

`scripts/prepare_scalar_eval.py` froze a 214-row workload from pinned BFCL sources (`data/bfcl/scalar_eval.jsonl`, provenance alongside): 58 scalar-schema positives, 100 no-tool live-irrelevance prompts with an offered scalar distractor, 25 atomic controls, 31 open-text controls. `experiments/benchmark_scalar_early_exit.py` interleaved four policies (`official`, `atomic_only`, `scalar_2`, and a `scalar_1` one-draft ablation on the positive/no-tool subsets) through one loaded BF16 checkpoint, no LoRA. The run completed its full 800-row trace: `experiments/scalar_exit_20260923T092825Z.jsonl`.

- The shipped `scalar_2` setting did **not** clear its own pre-registered bar: mean saving vs. `atomic_only` was statistically indistinguishable from zero both overall (+10.8 ms, 95% CI [-3.5, +31.0]) and on scalar positives alone (+27.6 ms, CI [-1.4, +85.5]).
- A `scalar_1` (one-draft) ablation did show a real effect on scalar positives: +197.2 ms mean saving (14.4% relative), CI [136.1, 276.7] ms, reproducing the same "extra stable draft erases benefit" pattern already seen in the atomic 1-vs-2-vs-3-draft ablation.
- Across every tested row at both N, the gate's returned tool call/content exactly matched what `atomic_only`/`official` independently produced for the same request (`matching_content_and_call`: 214/214 at N=2, 158/158 at N=1) — zero regressions introduced by gating.
- Five scalar exits disagreed with the strict-field gold score (1/33 on positives, 4/4 on no-tool distractors). Manually traced: in all five cases the un-gated official sampler made the identical error independently, so the gate reproduced a pre-existing base-model mistake faster rather than causing a new one — the same failure mode already documented for the atomic gate (4/43 wrong exits there), and at a similar absolute rate (4/100 here).
- The atomic-control bucket replicated the earlier matched atomic result almost exactly (28.6% mean reduction here vs. 28.3% previously), a useful sanity check that the hardware/harness are behaving consistently run to run.

`REFLEX_SCALAR_EARLY_EXIT` stays `false` by default: the shipped two-draft setting has no measured benefit to justify shipping it, and the one-draft variant needs a larger calibration/negative set before it could be defaulted on. Full tables, the raw trace, and the measured figure are in `RESULTS.md` and `SCALAR_EXPERIMENT.md`.
